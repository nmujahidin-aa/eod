...
func (s *service) EndOfDay(ctx context.Context) error {
	...
	for index, beforeEod := range beforeEodList {
		wg.Add(1)
		tempIndex := index
		go workerPoolProcessOne.AddTask(func(workerId int32) {
			defer wg.Done()
			mutex.Lock()
			afterEodList[tempIndex].AverageBalanced = (beforeEod.Balanced + beforeEod.PreviousBalanced) / 2
			afterEodList[tempIndex].ProcessOne = int64(workerId)
			mutex.Unlock()
		})
	}
	wg.Wait()
	...
	for index, beforeEod := range beforeEodList {
		wg.Add(1)
		tempIndex := index
		if beforeEod.Balanced <= 150 {
			go workerPoolProcessTwoA.AddTask(func(workerId int32) {
				defer wg.Done()
				mutex.Lock()
				afterEodList[tempIndex].FreeTransfer = 5
				afterEodList[tempIndex].ProcessTwoA = func() *int64 {
		...
		go workerPoolProcessTwoB.AddTask(func(workerId int32) {
			defer wg.Done()
			mutex.Lock()
			afterEodList[tempIndex].Balanced += 25
			afterEodList[tempIndex].ProcessTwoB = func() *int64 {
				workerId64 := int64(workerId)
				return &workerId64
			}()
			mutex.Unlock()
